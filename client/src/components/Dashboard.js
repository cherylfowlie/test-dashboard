...
const Standup = props => (
  <div>
    <tr>
      ...
      <td>
        <Link to={"/edit/" + props.standup._id}>edit</Link> | <a href="#" onClick={() => { props.deleteStandup(props.standup._id) }}>delete</a>
      </td>
    </tr>
  </div>
)
...
export default class StandupList extends Component {
  ...
  componentDidMount() {
    axios.get('http://localhost:3000/standup/')
      .then(response => {
  ...
  deleteStandup(id) {
    axios.delete('http://localhost:3000/standup/' + id)
  ...
  standupList() {
    return this.state.standup.map(currentstanduplist => {
      return <Standup standup={currentstanduplist} deleteStandup={this.deleteExercise} key={currentstanduplist._id} />;
    })
  }
